...
func getTeacherIdFromToken(c *gin.Context, db *gorm.DB) uint32 {
	var token string
...
func getTeacherId(c *gin.Context, db *gorm.DB) uint32 {
	adminId := getTeacherIdFromToken(c, db)
	//if adminId == 0 {
...
func TeacherVerifyHandle(c *gin.Context) {
	db := c.MustGet(constant.ContextDb).(*gorm.DB)
	adminId := getTeacherId(c, db)
	if adminId == 0 {
		c.AbortWithStatus(http.StatusForbidden)
		return
	}
	var teacher dbmodel.Teacher
	if err := db.Where("id = ?", adminId).First(&teacher).Error; err != nil {
		logger.Error(err)
		c.AbortWithStatus(http.StatusForbidden)
		return
	}
	c.Set(constant.ContextTeacher, teacher)
	c.Next()
}
